...
# Optional Bonus
#
# Efficiently handle inputs that are unsorted and much larger, both in the
# number of dice and in the number of sides per die. (For the purpose of this
# bonus challenge, you want the maximum value of some number k, times the
# number of times k appears in the input.)
#
# yahtzee_upper([1654, 1654, 50995, 30864, 1654, 50995, 22747,
#     1654, 1654, 1654, 1654, 1654, 30864, 4868, 1654, 4868, 1654,
#     30864, 4868, 30864]) => 123456
#
# There's no strict limit on how fast it needs to run. That depends on your
# language of choice. But for rough comparison, my Python solution on this
# challenge input, consisting of 100,000 values between 1 and 999,999,999 takes
# about 0.2 seconds (0.06 seconds not counting input parsing).
#
# If you're preparing for a coding interview, this is a good opportunity to
# practice runtime complexity. Try to find a solution that's linear (O(N))
...
def yahtzee_upper(roll):
    ...
    # Fetch unique numbers from list(roll)
    for NUMBER in set(roll):
        # Multiply the number of appearences by the number itself.
        if NUMBER * roll.count(NUMBER) > TOP_SCORE:
            # If the number is higher than top score, save that number.
            TOP_SCORE = NUMBER * roll.count(NUMBER)
